...
class InputForm(tk.Frame):
    ...
    def build_form(self):
        # --- City Input ---
        tk.Label(self, text="City:").grid(row=0, column=0, sticky="w", pady=(5, 2))
        self.city_entry = tk.Entry(self)
        self.city_entry.grid(row=0, column=1, pady=(5, 2))

        # --- Date Dropdown (Next 4 days) ---
        tk.Label(self, text="Date:").grid(row=1, column=0, sticky="w", pady=2)
        today = datetime.now()
        self.date_options = [
            (today + timedelta(days=i)).strftime("%Y-%m-%d") for i in range(4) # Next 4 days
        ]
        self.date_var = tk.StringVar(value=self.date_options[0])
        self.date_menu = tk.OptionMenu(self, self.date_var, self.date_options[0], *self.date_options)
        self.date_menu.grid(row=1, column=1, pady=2)

        # --- Time Input (HH:MM 24h) ---
        tk.Label(self, text="Time (24h - HH:MM):").grid(row=2, column=0, sticky="w", pady=2)
        self.time_entry = tk.Entry(self)
        self.time_entry.insert(0, "17:00")
        self.time_entry.grid(row=2, column=1, pady=2)

        # --- Gender Dropdown ---
        tk.Label(self, text="Gender:").grid(row=3, column=0, sticky="w", pady=2)
        self.gender_var = tk.StringVar(value="Female")
        gender_options = ["Male", "Female", "Nonbinary"]
        self.gender_menu = tk.OptionMenu(self, self.gender_var, gender_options[1], *gender_options)
        self.gender_menu.grid(row=3, column=1, pady=2)

        # --- Level Dropdown (1–20) ---
        tk.Label(self, text="Level:").grid(row=4, column=0, sticky="w", pady=2)
        self.level_var = tk.IntVar(value=1)
        level_options = list(range(1, 21))
        self.level_menu = tk.OptionMenu(self, self.level_var, level_options[0], *level_options)
        self.level_menu.grid(row=4, column=1, pady=2)

        # --- Submit Button ---
        submit_btn = tk.Button(self, text="Generate Character", command=self.submit)
